...
currently favored scenario of the early universe and refers to an
accelerated expansion of the spacetime in its earliest instant. See
...
field values close to $$\phi_*$$ (see figure).
...
away from everything than can be usually observed in Cosmology. In
fact, we can associate some length scales with it by counting how much
...
inflation imply that quantum physics is in charge not only within the
...
amount of time. However, we are living for sure in one of the
realizations in which stochastic inflation ended. As such, when
concerned with cosmological observables, we should be only concerned
...
time**: we now force the quantum field to start at
$$(\phi_{\mathrm{qw}},N_{\mathrm{qw}})$$ and to evolve backward in
times to end its evolution at $$(\phi_0,N_0)$$. Even is the idea is
...
stochastic inflation by reversing stochastic inflation from the
lifetime. Defining the reverse time by

$$\Delta N \equiv N_{\mathrm{qw}} - N,$$

Together with the field value in reference to the quantum wall $$\Delta
\phi \equiv \phi - \phi_{\mathrm{qw}}$$, the following picture shows
one of these reverse-time realization of stochastic inflation.
...
solution being represented below.

![revprob](/assets/images/2504.17680/revprob.png "Reversed-time probability distribution")

---

In such a reverse-time stochastic inflation formalism, all divergences
...
including the infinite limit. We find that it does only depend on one
parameter, the initial field excursion value $$\Delta \phi_0 =
...
It is skewed towards positive values and has tails decaying slowly as
$$|\zeta|^{-3/2}$$. In fact, the orange curve is an analytical
approximation matching very well the exact distribution in the tails. It reads
